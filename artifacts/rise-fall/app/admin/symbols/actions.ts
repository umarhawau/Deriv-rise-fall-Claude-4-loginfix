...
export async function applyDirectionRecommendationAction(
  ...
  if (!await isAdminAuthorized()) return { ok: false, error: 'Unauthorized' };
...
export async function toggleDirectionAction(
  ...
  await db
    ...
    .values({
      symbol,
      callEnabled: direction === 'CALL' ? !currentEnabled : true,
      putEnabled: direction === 'PUT' ? !currentEnabled : true,
      updatedAt: new Date(),
    })
    .onConflictDoUpdate({
      target: symbolConfigTable.symbol,
