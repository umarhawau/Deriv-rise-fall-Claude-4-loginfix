...
import { PositionsSheet } from '@/components/custom/positions-sheet';
import type {
  AuthState,
  DerivAccount,
  ActiveSymbol,
...
import type { UseAutotradeReturn } from '@/hooks/use-autotrade';
...
export interface RiseFallViewProps {
  ...
  requestAPI: UseSmartChartsApiReturn['requestAPI'];
...
export function RiseFallView({
  ...
  return (
    <main className="flex flex-col bg-background max-lg:h-dvh lg:overflow-visible">
              ...
              <Card className="lg:h-[min(33.6rem,66vh)] lg:min-h-[384px] lg:overflow-y-auto">
                <CardContent className="pt-4 space-y-4">
                  <TradeControls
                    ...
                    onViewPositions={() => setPositionsOpen(true)}
                  />
                  <AutotradePanel
                    autotrade={autotrade}
                    isAuthenticated={authState === 'authenticated'}
                    isConnected={isConnected}
                    userToken={activeAccount?.account_id ?? null}
                    activeSymbol={activeSymbol}
                  />
                </CardContent>
              </Card>
      ...
      {/* Mobile bottom navigation — Trade | Positions */}
      {authState === 'authenticated' && (
        <nav className="lg:hidden fixed bottom-0 inset-x-0 z-30 bg-background/95 backdrop-blur-sm border-t border-border flex">
          <button
            type="button"
            onClick={() => setPositionsOpen(false)}
            className="flex-1 flex flex-col items-center py-3 gap-0.5 text-muted-foreground"
          >
          ...
          <button
            ...
            className={cn(
              ...
              positionsOpen ? 'text-rose-500' : 'text-muted-foreground'
            )}
          >
            <div className="relative">
              <Clock className={cn('h-5 w-5', positionsOpen ? 'text-rose-500' : '')} />
              {openPositions.length > 0 && (
                <span className="absolute -top-1.5 -right-2 min-w-[16px] h-4 rounded-full bg-rose-500 text-[9px] font-bold text-white flex items-center justify-center px-0.5 leading-none">
                  {openPositions.length}
                </span>
              )}
            </div>
            <span className={cn('text-[11px] font-medium', positionsOpen ? 'text-rose-500' : '')}>Positions</span>
          </button>
        </nav>
      )}

      {/* Positions bottom sheet — stays in-page so the bot never stops */}
      <PositionsSheet
        open={positionsOpen}
